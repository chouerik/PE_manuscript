# Lineages Distribution Models through the partitioning of species' suitability values by lineages occurrences points.
# The original version of the script was developed by Rosauer et al (2015) and is available at https://github.com/DanRosauer/phylospatial/tree/master/LineageModels/multi-lineage range method.r
...
# install.packages(c('raster', 'sp', 'stringr', 'gdistance')
rm(list=ls())
library(raster)
library(sp)
library(stringr)
...
############## START OF PARAMETERS ##############
base_dir            <- '' #set '03_lineages_distribution_models' folder path here
SDMs_folder <- paste0(base_dir,'ldm_example_data/sdms_results')

files_list = list.files(paste(SDMs_folder, sep="/"), include.dirs = TRUE)
species_list = word(gsub("_"," ",files_list), 1,2, sep=" ")
genus_list = word(gsub("_"," ",files_list), 1, sep=" ")
epiteto_list= word(gsub("_"," ",files_list), 2, sep=" ")
setwd(base_dir)
dir.create(paste0(base_dir,'/output_ldm')) 
dir.create('output_ldm/misaligned') # output folder

target_dir          <- paste0(base_dir, 'output_ldm/misaligned') # where the lineage model grids and working data go
...
grid_resolution     <- 0.04166667 # copied from the resolution of the models
spRef               <- "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
output_model_extent <- extent(-82.00014, -42.50014, -23.00014, 13.49986)    # the maximum extent for all lineage models
...
min_SDM_value       <- 0.01  # values below this for the SDM are set to zero, to simplify calculation in areas of essentially unsuitable habitat
...
#i=1
  ...
  lineage_site_filename <- paste0(base_dir, 'ldm_example_data/lineages_occ/', genus_list[i],'_',epiteto_list[i],"_lin_occ_LDM.csv")

  all_lineages_sites <- read.csv(lineage_site_filename, sep="\t")
 ...
 cat ("\nModel groups to do in", genus_list[i], "\n")
  for (group in groups) {
    cat("\t", group, "\n")
  }
  ...
  for (group in groups) {
    ...
    cat ("\nStarting group", genus_list[i], group, "on", date(), "\n")

    # load the SDM model raster
    setwd(paste0(base_dir, 'ldm_example_data/sdms_results'))
    SDM_model <- dir(pattern = "cloglog.tif") 
    ...
    for (z in 1:nlayers(model.stack)) {
      model.ras <- model.stack[[z]]
      model_filename <- paste("LDM", "_", group, "_", model_names[z], ".asc", sep="")
    ...
    cat ("\nLineage models done for", genus_list[i], "\n")
