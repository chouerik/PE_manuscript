# This script was used to infer LR, PD, LWE, and PE surfaces in Choueri et al (in prep), as well the residual surfaces of LR/PD and LWE/PE.
# The original scripts writed by Rosauer et al., 2015 are available at https://github.com/DanRosauer/phylospatial/tree/master/LineageModels/.
...
#install.packages(c('raster', 'SDMTools', 'ape', 'phylobase', 'foreach', 'doParallel', 'ggplot2'))
rm(list=ls())
...
path <- '' # set '04_phylogeographic_endemism_diversity' folder path here
source(paste0(path,'04_phylogenetic_endemism.r'))
...
max.rows        <- 10000000
core_count      <- 3 # number of cores to use for parallel steps
...
#define directories
base.dir        <- path # modify to the base directory for your lineage modelling
input.dir       <- paste0(base.dir, 'phylo_end_div_example_data/lineages_distribution_models_aligned') #the output of lineages distribution models aligned for the analysis extent
output.dir      <- paste0(base.dir, 'output_phylo_surfaces')  # output location where diversity results and maps will be saved
dir.create(paste0(base.dir, 'output_phylo_surfaces'))

file_pattern    <- ''                     # modify this to a match the start of the name of all lineage model asc files

template_grid   <- paste0(base.dir, 'phylo_end_div_example_data/extent/extent.asc')
group_lin_file  <- paste0(base.dir, 'phylo_end_div_example_data/lineage_list/group_lin_list_example.csv')

#tree details  - this works for one genus at a time
tree.file       <-  paste0(base.dir, 'phylo_end_div_example_data/lineage_tree/Lineage_tree_example.tree')
outgroup        <- ''
preface         <- ''
...
setwd(base.dir)
group_lin_list <-read.csv(group_lin_file, stringsAsFactors=F, header = TRUE)
...
for (i in 1:nTips(tree)) {
  cat(labels(tree)[i],"\n")
  row <- group_lin_list[group_lin_list$model_group==labels(tree)[i],]
  if (nrow(row) > 0) {
...
# Infer residuals surfaces based in Fenker et al., 2020.
...
dataframe2asc(pos_output_log1[,c(4,3,10)],paste(output_prefix,"PD_SR_loglog_resid.asc",sep=""),output.dir)

# make some output images and files
setwd(output.dir)
...
PD_SR_resid.ras <- raster(output_filename1)
